          ...
          for (unsigned int v = 0; v < GeometryInfo<2>::vertices_per_cell; ++v)
            {
              ...
              if (std::fabs(distance_from_center - inner_radius) < 2e-10)
                {
...
// @sect3{Creating a third mesh}

// The grid in the following, third function generates a circle.
void third_grid()
{
  // We start again by defining an object for a triangulation of a
  // two-dimensional domain:
  ...
  // We then fill it with a circular domain. The center of the circle shall be the
  // point (1,0), and the radius will be 0.45. The number of
  // circumferential cells could be adjusted automatically by this function
  const Point<2> center(1.0, 0);
  const double   radius = 0.45;
  GridGenerator::hyper_ball(
    triangulation, center, radius,1);
...
// @sect3{Creating a fourth mesh}

// The grid in the following, third function generates a L-shape.
void fourth_grid()
{
  // We start again by defining an object for a triangulation of a
  // two-dimensional domain:
  ...
  // for one single global refinement:
  //triangulation.refine_global(1);
  ...
  for (unsigned int step = 0; step < 5 ; ++step)
    {
	  for (auto &cell : triangulation.active_cell_iterators())
	  {
        ...
        const double distance_from_center =
               center.distance(cell->vertex(v));
               // for 5 adaptative refinements for distance to the center < 0.3 use :
               //if ((std::fabs(distance_from_center ) <0.3 )
               // for 5 adaptative refinements at the re-entrant corner :
               if ((std::fabs(distance_from_center ) <0.3 && step==0 ) || std::fabs(distance_from_center ) < 2e-10)
               {
                  cell->set_refine_flag();
               }
    	}
