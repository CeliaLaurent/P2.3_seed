  ...
  for (; cell!=endc; ++cell)
    {
      ...
      constraints.distribute_local_to_global( // void
    		  cell_matrix,       // const FullMatrix< number > &local_matrix,
			  cell_rhs,          // const Vector< number > &local_vector,
              local_dof_indices, // const std::vector< size_type > &local_dof_indices,
			  system_matrix,     // MatrixType &global_matrix,
			  system_rhs         // VectorType &global_vector,
			  );                 // bool use_inhomogeneities_for_rhs=false)
      // above distribute_local_to_global substitutes the below global affinement:      
//    for (unsigned int i=0; i<dofs_per_cell; ++i)
//      for (unsigned int j=0; j<dofs_per_cell; ++j)
