...
// now include distributed-data specific librarires:
...
//#define USE_PESC_LA // comment to use Trilinos instead of PETSc
...
namespace LA
{
#ifdef USE_PETSC_LA   // use either PERSc :
...
private:
  ...
  // associates constraints of the triangulation
...
Step3<dim>::Step3 ()
  ...
  // enable pout only if this_mpi_process ==0 so that only rank 0 prints to std::cout
  ...
  timer(pout, TimerOutput::summary,
...
void Step3<dim>::estimate_error ()
{
  ...
  KellyErrorEstimator<dim>::estimate(
            ...
            locally_relevant_solution, // field whose error must be stored on the error estimator
            error_estimator);
}
...
void Step3<dim>::assemble_system ()
{
  ...
  //auto worker_function=[dofs_per_cell,n_q_points,exact_rhs_function](
      ...
      for (unsigned int q_index=0; q_index<n_q_points; ++q_index)
        {
          ...
          for (unsigned int i=0; i<dofs_per_cell; ++i)
              copy_data.vectors[0](i) += (fe_values.shape_value (i, q_index) *
                            exact_rhs_function.value(q_points[q_index]) * 
                            fe_values.JxW (q_index));
        }
      cell->get_dof_indices (copy_data.local_dof_indices[0]);
  };
  auto copier_function=[&](
               const MeshWorker::CopyData<1,1,1> & copy_data)
  {
      constraints.distribute_local_to_global(   
                          copy_data.matrices[0],
                          copy_data.vectors[0],          
  ...
  // run worker and copier functions using threads:
  WorkStream::run(//dof_handler.active_cell_iterators() ,
    ...
    CellFilter(IteratorFilters::LocallyOwnedCell(),dof_handler.end()),
                  ...
                  scratch,
                  copy_data);
  // which replaces the serial loop:
  // for (auto cell: dof_handler.active_cell_iterators())
  //  { 
  //    worker_function(cell,scratch,copy_data);
  //    copier_function(copy_data);
  //  }
...
#ifdef USE_PETSC_LA   //  with PETSc :
  data.symmetric_operator=true;
#else  // or Trilinos :
  // no need for Trilinos, default is good.
#endif
  amg_preconditionner.initialize(system_matrix);

  solver.solve (system_matrix, solution, system_rhs,
                amg_preconditionner); //PreconditionIdentity());
...
int main (int argc,char **argv)
{
  Utilities::MPI::MPI_InitFinalize mpi_initialization ( 
            argc,  // int &  	argc,
            argv,  // char **&  	argv,
            // to use the maximum number of threads use max_num_threads=-1 :
            -1  // const unsigned int  	max_num_threads = numbers::invalid_unsigned_int 
	) ;
